...
function WaitForClick(frameParent,polledFunction,exitFunction)
	...
	connection=Mouse.Button1Up:connect(function(nx,ny)
		exitFunction(nx,ny)
		BigButton.Visible=false
...
function RemoveAllStats(playerEntry)
	for i,val in ipairs(ScoreNames) do
		StatRemoved(ScoreNames,playerEntry)
	end
...
function StartDrag(entry,startx,starty)
	local startDragTime = tick()
		...
		local function dragExit() 
			debugprint('undone click'.. tick()-startDragTime) 
			stopDrag = true 
			if openPanel and entry['Player']~=LocalPlayer and SelectedPlayer.userId>1 and LocalPlayer.userId>1 then
