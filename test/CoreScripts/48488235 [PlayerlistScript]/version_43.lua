...
if not personalServerPlace then -- one more backup check
	...
	if theBool and theBool:IsA("BoolValue") then
		...
		wait(0.2)
		theBool = game.Workspace:FindFirstChild("PSVariable")
		personalServerPlace = (theBool and theBool:IsA("BoolValue"))
	end
end
...
if personalServerPlace then
	addContextMenuButton("Ban", 
		--IsVisible
		function(player) 
			return ( getPrivilegeType(game.Players.LocalPlayer) >= privilegeAdmin and (getPrivilegeType(player) < privilegeOwner) )
		end, 
		--IsActive
		function(player) 
			return true 
