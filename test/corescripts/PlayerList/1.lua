...
local smallWindowPosition = UDim2.new(0, 0, 0,0)
local smallWindowSize = UDim2.new(1,5,1,0)
...
local container = nil
local bottomRightTrayContainer = nil
...
local function rebuildBoard(owner, boardType, numStats)
	print("RebuildBoard")
	if bottomRightTrayContainer == nil then
		bottomRightTrayContainer = owner:FindFirstChild("PlayerListBottomRightFrame")
		if bottomRightTrayContainer == nil then
			bottomRightTrayContainer = Instance.new("Frame")
			bottomRightTrayContainer.Name = "PlayerListBottomRightFrame"
			bottomRightTrayContainer.BackgroundTransparency = 1
			bottomRightTrayContainer.Size = UDim2.new(0.2, 0, 0.42, 0)
			bottomRightTrayContainer.Position = UDim2.new(0.8, 0, 0.5, 0)
			bottomRightTrayContainer.Parent = container
		end
	end
	if minimizedState == nil then
		...
		minimizedState.Position = UDim2.new(1, -153, 1, -30)
		...
		minimizedState.Parent = bottomRightTrayContainer
	end
	...
	if container then
		if bottomRightTrayContainer then
			--Delete the old boards
			if bottomRightTrayContainer:FindFirstChild("SmallPlayerlist") then
				smallVisible = bottomRightTrayContainer.SmallPlayerlist.Visible
				bottomRightTrayContainer.SmallPlayerlist.Parent = nil
	...
	if smallBoard then
		...
		smallBoard.Parent = bottomRightTrayContainer
	end
	if bigBoard then
		if bigVisible then
...
local function hideSmallPlayerWindow(completed)
	...
	if bottomRightTrayContainer:FindFirstChild("SmallPlayerlist") then
		local completedFunction = completed
		if bottomRightTrayContainer.SmallPlayerlist.Visible then
			bottomRightTrayContainer.SmallPlayerlist:TweenPosition(UDim2.new(1,0,smallWindowPosition.Y.Scale, smallWindowPosition.Y.Offset), Enum.EasingDirection.Out, smallEasingStyle, 0.3, true, 
				function(state) 
					--print(state)
					if state == Enum.TweenStatus.Completed then 
						if bottomRightTrayContainer:FindFirstChild("SmallPlayerlist") then
							bottomRightTrayContainer.SmallPlayerlist.Visible = false
						end
						if completedFunction then
							completedFunction()
						end
...
transitionWindowsFunction = function(desiredState)
	...
	elseif desiredState == "Small" then
		...
		hideBigPlayerWindow()
		if bottomRightTrayContainer:FindFirstChild("SmallPlayerlist") then
			if not bottomRightTrayContainer.SmallPlayerlist.Visible or bottomRightTrayContainer.SmallPlayerlist.Position ~= smallWindowPosition then
				bottomRightTrayContainer.SmallPlayerlist.Visible = true
				bottomRightTrayContainer.SmallPlayerlist:TweenPosition(smallWindowPosition, Enum.EasingDirection.Out, smallEasingStyle, 0.3, true)
			end
		end
...
if UserSettings and LoadLibrary then
	...
	local function createStatColumn(i, numColumns, isTeam, color3, isHeader)
		...
		textLabel.Position = UDim2.new((i-1) * (1/numColumns), 2, 0, 0)
	...
	local function buildPlayerObject(player, numStatColumns, suffix)
		...
		if supportFriends and player ~= game.Players.LocalPlayer then
				...
				button.MouseEnter:connect(
					function()
						if previousTransparency == nil then
							previousTransparency = button.BackgroundTransparency
						end
						...
						if lightBackground then
							...
							button.BackgroundTransparency = 0.80
						end
					end))
			table.insert(playerTable[player].Connections,
			...
			table.insert(playerTable[player].Connections,
				button.MouseButton1Click:connect(function() 
					showPlayerMenu(player, mouseDownX, button.AbsolutePosition.Y + button.AbsoluteSize.Y )
				end))
	...
	local function createPlayerListBasics(frame, isBig)
		...
		if isBig then
			scrollFrame.Position = UDim2.new(0,-7,0,0)
			scrollUp.Position = UDim2.new(1,-19,0,0)
			scrollDown.Position = UDim2.new(1,-19,1,-15)
			scrollBar.Position = UDim2.new(1, -19, 0, 19)

			scrollFrame.Size = UDim2.new(1,-20,1,0)
			headerFrame.Size = UDim2.new(1,-20,0,32)
		else
			scrollBar.Position = UDim2.new(1, -17, 0, 19)
			scrollFrame.Position = UDim2.new(0,-5,0,0)
			scrollUp.Position = UDim2.new(1,-17,0,0)
			scrollDown.Position = UDim2.new(1,-17,1,-15)
	...
	createBoardsFunction = function (boardType, numStatColumns)
		...
		playerListButton.Position = UDim2.new(1, -3, 0.5, -15)
		playerListButton.MouseButton1Click:connect(
			function()
				toggleBigWindow()
			end)
