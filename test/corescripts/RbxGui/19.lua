...
local function setSliderPos(newAbsPosX,slider,sliderPosition,bar,steps)
	...
	local relativePosX = math.max(0, (newAbsPosX - bar.AbsolutePosition.X) / bar.AbsoluteSize.X )
	relativePosX = math.min(1, relativePosX)
	...
	if sliderPosition.Value ~= result then 
		sliderPosition.Value = result
	end
