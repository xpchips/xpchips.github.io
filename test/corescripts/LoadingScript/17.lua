...
-- need this here ASAP
CP:Preload(IMAGES.GAME_THUMBNAIL)
...
function MainGui:RecalculateSizes(screenGui)
	...
	if InfoProvider:GetCreatorName() == '' or InfoProvider:GetGameName() == '' then
		Spawn(function()
			while InfoProvider:GetCreatorName() == '' or InfoProvider:GetGameName() == '' do
				wait()
			end
		...
		screenGui.MainBackgroundContainer.BottomBar.BottomBarActual.TextContainer.CreatorName.Text = creatorName
		screenGui.MainBackgroundContainer.BottomBar.BottomBarActual.TextContainer.GameName.Text = gameName
	end
...
renderSteppedConnection = Game:GetService("RunService").RenderStepped:connect(function()
	...
	if voxelCount <= 0 then
		...
		currScreenGui.ThumbnailContainer.LoadingInfoContainer.VoxelsValue.Text = tostring(voxelCount) .." millions"
	end
end)
