...
function selectChoice(choice)
	...
	if choice == lastChoice then
		...
		game.Chat:Chat(game.Players.LocalPlayer.Character, dialogChoice.UserDialog, getChatColor(currentTone()))
		wait(1)
		currentConversationDialog:SignalDialogChoiceSelected(player, dialogChoice)
		game.Chat:Chat(currentConversationPartner, dialogChoice.ResponseDialog, getChatColor(currentTone()))
...
function doDialog(dialog)
	while not Instance.Lock(dialog) do
		wait()
	end
...
function waitForChild(instance, name)
	while not instance:FindFirstChild(name) do
		instance.childAddedSignal:wait()
	end
end
...
function addDialog(dialog)
	if dialog.Parent and dialog.Parent:IsA("BasePart") then
      ...
      dialog.Changed:connect(function(prop)
          if prop == "Parent" and dialog.Parent then 
              removeDialog(dialog) addDialog(dialog) 
...
function fetchScripts()
	--timeoutScript = script.TimeoutScript;
...
function onLoad()
  ...
  createMessageDialog()
  messageDialog.Parent = screenGui
