...
function centerGear(equipped)
	local size = 0
	for i = 1, #gearSlots do
		if gearSlots[i] ~= "empty" and gearSlots[i]:IsA("ImageButton") and type(gearSlots[i].Name) == "string" then
			size = size + 1
		end
	end
	...
	local totalWidth
	local startPos
	if size == maxNumLoadoutItems then
		startPos = 0
	else
		totalWidth = currentLoadout.TempSlot.Size.X.Scale * size
		startPos = (1 - totalWidth)/2
	end
	
	normalizeAllButtons()
	wait(0.1)

	delay(guiTweenSpeed + 0.9,function()
		enlargeOverride = true
		if waitingOnEnlarge then
			enlargeButton(waitingOnEnlarge)
			waitingOnEnlarge = nil
		end
	end)
end
...
function insertGear(gear, addToSlot)
	...
	if pos ~= maxNumLoadoutItems then
		if(type(tostring(pos)) == "string") then gear.SlotNumber.Text = tostring(pos) end
...
function toolSwitcher(numKey)
	...
	for i = 1, #gearSlots do
		if gearSlots[i] ~= "empty" and i ~= key then
...
function swapGear(gearClone,toFrame)
	...
	if #toFrameChildren == 1 then
		if toFrameChildren[1]:FindFirstChild("SlotNumber") then
			...
			gearClone.SlotNumber.Text = string.sub(toFrame.Name,5)
			
			gearClone.Position = UDim2.new(0,0,0,0)
			toFrameChildren[1].Position = UDim2.new(0,0,0,0)
			
			toFrameChildren[1].Parent = gearClone.Parent
			gearClone.Parent = toFrame
		end
	else
		local slotNum = gearClone.SlotNumber.Text
		if slotNum == 0 then slotNum = 10 end
		table.remove(gearSlots,slotNum)
...
local addingPlayerChild = function(child, equipped, addToSlot, inventoryGearButton)
	...
	dragBegin = gearClone.DragBegin:connect(function(pos)
		...
		for i = 1, #children do
			if children[i]:IsA("Frame") or children[i]:IsA("ImageLabel") or children[i]:IsA("TextLabel") then
				children[i].ZIndex = 7
			end
		end
	end)
...
for i = 1, size do
	addingPlayerChild(backpackChildren[i],true)
end
