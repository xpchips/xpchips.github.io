...
ADMINS['erik.cassel'] = 1
ADMINS['david.baszucki'] = 1
...
local RecordedMessageBox = Obj.Create'Frame'
{
	...
	Obj.Create'TextLabel'
	{
		...
		BackgroundTransparency = 1,
		TextColor3 = Color3.new(1,1,1),
		Text = 'We have recorded your report for evaluation.',
		TextWrapped = true,
		TextYAlignment = Enum.TextYAlignment.Top,
...
function OnSubmitAbuse()
	if SubmitReportButton.Active then
		if AbuseName and SelectedPlayer then
			...
			if AbuseName == "Cheating/Exploiting" then
				RecordedMessageBox.Parent = ReportAbuseShield
			elseif AbuseName == "Bullying" or AbuseName == "Swearing" then
...
function InitReportAbuse()
	...
	RecordedMessageBox:FindFirstChild('OkButton').MouseButton1Down:connect(CloseAbuseDialog)
end
...
function PlayerSortFunction(a,b)
 -- prevents flipping out leaderboard
	if a['Score'] == b['Score'] then
		--return a['ID'] < b['ID']
		return a['Player'].Name:upper() < a['Player'].Name:upper()
	end
...
function ExpandNames()
	--print('enered expand names')
	if #ScoreNames ~= 0 then
		for _,i in pairs(StatTitles:GetChildren()) do
			print (tostring(#ScoreNames) .. " is expanding")
...
function ClosePopUpPanel()
	if SelectedPlayerEntry then --on random times this will be called by both button and playermouse
		local tframe = SelectedPlayerEntry['Frame']
		Spawn(function() TweenBackgroundTransparency(tframe,.5,1,.25) end)
		PopUpPanel:TweenPosition(UDim2.new(1,0,0,0), "Out", "Quad", .5,true)
		wait(.1)
...
function InitMovingPanel( entry, player)
	...
	ReportPlayerButton.MouseButton1Click:connect(function() ClosePopUpPanel() OpenAbuseDialog() end)
	...
	PopUpPanel:TweenPosition(UDim2.new(0,0,0,0), "Out", "Quad", .5,true)
	Delay(1, function()
		local tconnection
		tconnection = LocalPlayer:GetMouse().Button1Down:connect(function()
...
function OnPlayerEntrySelect(entry)
	if SelectedPlayer then
		debugprint('trying to open popup menu!!'.. tostring(entry == SelectedPlayerEntry).. ' ' .. tostring(SelectedPlayer~=LocalPlayer)..' ' .. tostring(SelectedPlayer.userId) ..' '..tostring(LocalPlayer.userId)  )
	end
	if not InPopupWaitForClick and entry == SelectedPlayerEntry and SelectedPlayer~=LocalPlayer and SelectedPlayer  and SelectedPlayer.userId>1 and LocalPlayer.userId>1 then
		
		entry['Frame'].BackgroundColor3 = Color3.new(0,1,1)
		Spawn(function() TweenBackgroundTransparency(entry['Frame'],1,.5,.5) end)
		InPopupWaitForClick=true
		InitMovingPanel(entry,SelectedPlayer)
		
		
	elseif not InPopupWaitForClick then
		print('starting drag ', entry['Player'].Name)
