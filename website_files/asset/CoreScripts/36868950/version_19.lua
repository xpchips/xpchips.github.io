...
for i = 1, #bottomLeftChildren do
	...
	elseif bottomLeftChildren[i].Name == "SettingsButton" then
		hasSettingsTip = true
			print("inline settings")
		createSettingsButtonTip(bottomLeftChildren[i])
	end
end

if not hasSettingsTip then
	local descCon = nil
	descCon = bottomLeftControl.DescendantAdded:connect(function(instance)
		if instance.Name == "SettingsButton" then
			print("Event settings")
			createSettingsButtonTip(instance)
			descCon:disconnect()
		end
	end)
end
