...
local upgradeBCConnection =  nil 

function showPurchasePrompt()
	local canPurchase, insufficientFunds, hasRightBC = canPurchaseItem()	
	if hasRightBc and hasRightBc == 1 then 
		purchaseDialog.BodyFrame.AfterBalanceButton.Text = "You require an upgrade to your BuildersClub membership to purchase this item. Click here to upgrade" 
		if not upgradeBCConnection then 
			upgradeBCConnection = purchaseDialog.BodyFrame.AfterBalanceButton.MouseButton1Click:connect(function()
				if purchaseDialog.BodyFrame.AfterBalanceButton.Text == "You require an upgrade to your BuildersClub membership to purchase this item. Click here to upgrade"  then 
					openBCUpSellWindow()
				end 
			end)
		end 
...
function updateAfterBalanceText(playerBalance)
	if isFreeItem() then
	...
	elseif afterBalanceNumber < 0 and keyWord == "tickets" then
		return false -- user can't buy more tickets, so we say fail the transaction (maybe instead we can prompt them to trade currency???)
	end
...
function isFreeItem()
	...
	return currentProductInfo["IsForSale"] == true and currentProductInfo["IsPublicDomain"] == true
end
...
function canPurchaseItem()
	...
	if playerOwnsAsset == true or playerOwnsAsset == "true" then
		print("Player already owns asset")
		return false
	end
	...
	if currentProductInfo["ContentRatingTypeId"] == 1 then
		if game.Players.LocalPlayer:GetUnder13() then
			print("user is under 13 and cannot purchase this item")
			return false
		end
	end
	...
	if tonumber(currentProductInfo["MinimumMembershipLevel"]) > membershipTypeToNumber(game.Players.LocalPlayer.MembershipType) then
		print("player membership type is too low for purchase of this item!")
		return false, 0, 1
	end
	...
	if (currentProductInfo["IsLimited"] == true or currentProductInfo["IsLimitedUnique"] == true) and
		(currentProductInfo["Remaining"] == "" or currentProductInfo["Remaining"] == 0 or currentProductInfo["Remaining"] == nil) then
			print("No more of product is left for sale!")
			return false
	end
	...
	if not setCurrencyAmountAndType(tonumber(currentProductInfo["PriceInRobux"]), tonumber(currentProductInfo["PriceInTickets"])) then
		print("we could not set currency correctly")
		return false
	end

	local playerBalance = getPlayerBalance()
	if not playerBalance then
		print("could not get player balance!")
		return false
	end

	local updatedBalance, insufficientFunds = updateAfterBalanceText(playerBalance)

	if not updatedBalance then
		print("updateAfterBalanceText failed")
		return false
	end

	-- we use insufficient funds to display a prompt to buy more robux
	return true, insufficientFunds, notRightBc
end

-- make sure our gui displays the promper purchase data, and set the productid we will try and buy if use specifies a buy action
function updatePurchasePromptData()
