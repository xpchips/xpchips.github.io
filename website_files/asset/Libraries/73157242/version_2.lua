...
t.GetStampModel = function(assetId, terrainShape, useAssetVersionId)
	...
	local function getApproximateTerrainMaterialTextures(terrainValue)
		...
		local TERRAIN_TEXTURE_PATH = "http://www.roblox.com/asset/?id="
...
t.SetupStamperDragger = function(modelToStamp, Mouse, StampInModel, AllowedStampRegion, StampFailedFunc)
	...
	local function DoStamperMouseMove(Mouse)
		...
		if stampData.CurrentParts and (stampData.CurrentParts:FindFirstChild("ClusterMaterial",true) or stampData.CurrentParts.Name == "MegaClusterCube") 
			and HighScalabilityLine and HighScalabilityLine.Start then
	...
	local function DoStamperMouseDown(Mouse)
		...
		if stampData.CurrentParts and (stampData.CurrentParts:FindFirstChild("ClusterMaterial",true) or stampData.CurrentParts.Name == "MegaClusterCube") then
	...
	local function ResolveMegaClusterStamp(checkHighScalabilityStamp)
		...
		if not cluster then
			cluser = game.Workspace:FindFirstChild("Terrain")
		end
		
		if not cluster then
			return cellSet
		end
		if not cluster:IsA("Terrain") then
			return cellSet
		end
	...
	local function DoStamperMouseUp(Mouse)
		...
		if stampInModel then
			local canStamp = nil
			local isHSLPart = (stampData.CurrentParts:FindFirstChild("ClusterMaterial",true) or stampData.CurrentParts.Name == "MegaClusterCube")
		...
		if HighScalabilityLine.Start and HighScalabilityLine.Adorn.Parent and
			stampData.CurrentParts and (stampData.CurrentParts:FindFirstChild("ClusterMaterial",true) or stampData.CurrentParts.Name == "MegaClusterCube") then
		...
		if cluster and (stampData.CurrentParts:FindFirstChild("ClusterMaterial",true) or stampData.CurrentParts.Name == "MegaClusterCube") then
