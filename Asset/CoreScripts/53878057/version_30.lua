...
function removeAllEquippedGear(physGear)
	...
	for i = 1, #stuff do
		if ( stuff[i]:IsA("Tool") or stuff[i]:IsA("HopperBin") ) and stuff[i] ~= physGear then
			if stuff[i]:IsA("Tool") then doUnequipTools() end
...
function doEquipTool(toolToEquip)
	if not player.Character then return end

	local playerChildren = player.Character:GetChildren()
	for i = 1, #playerChildren do
		if playerChildren[i]:IsA("Humanoid") then
			equippingTool = true

			local childToolCon, charDiedCon = nil
			childToolCon = player.Character.ChildAdded:connect(function (child)
				if child == toolToEquip then
					equippingTool = false
					if childToolCon then childToolCon:disconnect() end
					if charDiedCon then charDiedCon:disconnect() end
				end
			end)
			charDiedCon = playerChildren[i].Died:connect(function (child)
				if child == toolToEquip then
					equippingTool = false
					if charDiedCon then charDiedCon:disconnect() end
					if childToolCon then childToolCon:disconnect() end
			...
			playerChildren[i]:EquipTool(toolToEquip)
			break
		end
	end
end
function doUnequipTools()
	if not player.Character then return end

	local playerChildren = player.Character:GetChildren()
	for i = 1, #playerChildren do
		if playerChildren[i]:IsA("Humanoid") then
			playerChildren[i]:UnequipTools()
			break
		end
	end
end
...
game:GetService("GuiService").KeyPressed:connect(function(key)
	if characterInWorkspace() and not equippingTool then
		activateGear(key)
	end
end)
