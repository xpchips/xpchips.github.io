...
local itemPurchaseText = "Would you like to buy the assetType 'itemName' for currencyTypecurrencyAmount?"
local productPurchaseText = "Would you like to buy 'itemName' for currencyTypecurrencyAmount?"
...
function createPurchasePromptGui()
	...
	titleLabel.TextColor3 = Color3.new(31/255,161/255,46/255)
	titleLabel.ZIndex = 8
...
function userPurchaseProductActionsEnded(userIsClosingDialog)
	...
	if userIsClosingDialog then
		...
		if tostring(currentServerResponseTable["isValid"]):lower() == "true" then
			...
			print("Server ticker response table was invalid, cancelling purchase")
			purchaseFailed()
		end
	end
end
