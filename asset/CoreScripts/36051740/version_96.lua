...
local buildToolsVisible = true
if buildTools.Frame.Position.X.Offset ~= 0 then
...
-- search for player value (may be in a couple of different places)

while tryCount < 10 do

	tryCount = tryCount + 1
	if script:FindFirstChild("PlayerName") ~= nil then
		location = 1
		tryCount = 10
	elseif script.Parent:FindFirstChild("PlayerName") ~= nil then
		location = 2
		tryCount = 10
	end
	wait(0.2)
...
local backpack
if location == 1 then
	player = game.Players:FindFirstChild(script.PlayerName.Value).Character
	backpack = game.Players:FindFirstChild(script.PlayerName.Value).Backpack
elseif location ==2 then
	player = game.Players:FindFirstChild(script.Parent.PlayerName.Value).Character
	backpack = game.Players:FindFirstChild(script.Parent.PlayerName.Value).Backpack
end
...
function removeHopper(descendant)

	if descendant:IsA("Tool") and equippedTool and equippedTool:IsA("HopperBin") then
		equippedTool.Active = false
		equippedTool.Parent = nil
		resetAllButtons()
	end	
	...
	elseif hasTool then
		...
		if tool then
			...
			elseif equippedTool then
				...
				player.DescendantAdded:connect(function(descendant) removeHopper(descendant) end)
			end
			setButtonStateActive(Object, Name)
		end
	end
...
buildTools.Frame.AddDeleteTools.InsertObject.MouseButton1Click:connect(function() 
	toolSelection("BuildInsert",buildTools.Frame.AddDeleteTools.InsertObject,36431591)
