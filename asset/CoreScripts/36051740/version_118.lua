...
-- Deprecated Tools
local buildInsertID = 58901335
local buildPaintID = 58901481
local buildResizeID = 58901534
local buildMaterialID = 58901405
local buildSurfaceID = 58901575
...
if not useNewBuildTools then
	useNewBuildTools = (game.PlaceId == 41647283)
end

-- Start Script
if useNewBuildTools then -- TODO: remove this when transition to new tools is done
	...
	function givePlayerBuildTools()
		...
		giveAssetId(buildGroupDraggerID)
		giveAssetId(buildRotateID)
		giveAssetId(buildConfigID)
		
		-- deprecated tools
		giveAssetId(buildInsertID)
		giveAssetId(buildPaintID)
		giveAssetId(buildResizeID)
		giveAssetId(buildMaterialID)
		giveAssetId(buildSurfaceID)
	end
...
else -- We are using old build tools
	...
	function loadTools()
		...
		if buildGroupDragger then buildGroupDragger.Parent = backpack end
		...
		local children = buildDelete:GetChildren()
		for i = 1, #children do
			if children[i]:IsA("Tool") then
				buildDelete = children[i]
				break
			end
		end
