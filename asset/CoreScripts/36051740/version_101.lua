-- this script is responsible for handling of build tools through buildtools bar
...
function loadTools()
	...
	buildStamper = game:GetService("InsertService"):LoadAsset(buildStamperID)
	if buildStamper then
		buildStamper = buildStamper:FindFirstChild("StamperTool")
		if buildStamper then
			giveSelectedValue(buildStamper)
			buildStamper.Parent = backpack
		end
	end
...
function setUpText(toolTip)
	...
	elseif name == "StampTool" then toolTip.Text = "Stamp Parts"
...
function reset(subset)
	...
	elseif subset.Name == "PhysicalTools" then
		for i = 1, #buttons do
			...
			elseif buttons[i].Name == "StampTool" then buttons[i].Image = localAssetBase .."StampToolButton.png"
...
function setButtonStateActive(Object, Name)
	...
	elseif Name == "StamperTool" then Object.Image = localAssetBase .. "StampToolButton_dn.png" Object.Selected = true
...
function getToolLocal(Name)
	...
	elseif Name == "buildStamper" then return buildStamper
...
pcall(function() buildTools.Frame.PhysicalTools.StampTool.MouseButton1Click:connect(function()
toolSelection("StamperTool",buildTools.Frame.PhysicalTools.StampTool,buildStamperID) end) end)
