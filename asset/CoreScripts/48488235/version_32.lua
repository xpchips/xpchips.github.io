	...
	local function updatePlayerContextMenu(player)
		...
		for i, contextElement in ipairs(contextMenuElements) do
			...
			if contextElement.IsVisible then
				...
				print("contextElement.Type =",contextElement.Type)
	...
	local function doSort(tableToSort, objectName, order, startPos, alphabetical)
		...
		for i, player in ipairs(tableToSort) do
			if playerTable[player] then
				...
				if not alphabetical then startPos = startPos + 1 end
			end
		end
		if alphabetical then
			table.sort(orderedPlayerTable,function(a,b) return (a.Name < b.Name) end)
			if playerTable[game.Players.LocalPlayer] and playerTable[game.Players.LocalPlayer][objectName] then
	...
	local function orderScrollList(scrollOrder, objectName, scrollFrame, alphabetical)
		...
		if isTeam then
			...
			for i, team in ipairs(teams) do
				...
				pos = pos + 1
				doSort(teamTable[team].Players, objectName, order, pos, alphabetical)
			end
...
if contextMenu3d then
	...
	function populateActions(scrollFrame, nullFrame, recalcFunction, otherPlayer)
		...
		for i = 1, #elements do
			print(" elements[i].Button.Visible =", elements[i].Button.Visible)
	...
	--player.userId = 7210880
	--player.Character = game.Workspace:FindFirstChild("The Noobish Noob")
