...
local function createGameSettingsMenu(baseZIndex, shield)
	...
	if hasGraphicsSlider then
		...
		local graphicsSlider, graphicsLevel = RbxGui.CreateSlider(settings().Rendering:GetMaxQualityLevel() - 1,150,UDim2.new(0, 230, 0, 280)) -- graphics - 1 because slider starts at 1 instead of 0
		...
		local graphicsChangedFrame = Instance.new("Frame")
		graphicsChangedFrame.RobloxLocked = true
		graphicsChangedFrame.Name = "GraphicsChangedFrame"
		graphicsChangedFrame.Style = Enum.FrameStyle.RobloxSquare
		graphicsChangedFrame.Size = UDim2.new(0,200,0,60)
		graphicsChangedFrame.Position = UDim2.new(1,-200,1,-120)
		graphicsChangedFrame.Visible = false
		graphicsChangedFrame.Parent = game:GetService("CoreGui").RobloxGui
		
		local graphicsChangedText = Instance.new("TextLabel")
		graphicsChangedText.RobloxLocked = true
		graphicsChangedText.Name = "GraphicsChangedText"
		graphicsChangedText.Font = Enum.Font.ArialBold
		graphicsChangedText.FontSize = Enum.FontSize.Size18
		graphicsChangedText.TextColor3 = Color3.new(1,1,1)
		graphicsChangedText.BackgroundTransparency = 1
		graphicsChangedText.TextWrap = true
		graphicsChangedText.Size = UDim2.new(1,0,1,0)
		graphicsChangedText.Text = ""
		graphicsChangedText.Parent = graphicsChangedFrame
		...
		graphicsSetter.FocusLost:connect(function()
			...
			settings().Rendering.QualityLevel = graphicsLevel.Value
			graphicsSetter.Text = tostring(graphicsLevel.Value)
			print("set graphics to newGraphicsValue = ",newGraphicsValue)
		end)
		...
		local function goToManualGraphics()
			...
			settings().Rendering.QualityLevel = settings().Rendering.AutoFRMLevel
			graphicsSetter.Text = tostring(graphicsLevel.Value)
		end
		...
		local function setupGraphicsChangedCallback()
			lastUpdate = tick()
			delay(3,function()
				local theTime = tick()
				if math.abs(lastUpdate + 3 - theTime) < 0.05 then -- allow some tolerance for tick(), right now this is 0.05
					graphicsChangedFrame.Visible = false
				end
			end)
		end
		...
		game.GraphicsQualityChangeRequest:connect(function(graphicsIncrease)
			...
			if graphicsIncrease then
				if (graphicsLevel.Value + 1) >= settings().Rendering:GetMaxQualityLevel() then return end
				graphicsLevel.Value = graphicsLevel.Value + 1
				graphicsSetter.Text = tostring(graphicsLevel.Value)
				settings().Rendering.QualityLevel = graphicsLevel.Value
				
				graphicsChangedFrame.Visible = true
				graphicsChangedText.Text = "Graphics Quality Increased (" .. graphicsSetter.Text .. ")"
				setupGraphicsChangedCallback()
			else
				...
				graphicsChangedFrame.Visible = true
				graphicsChangedText.Text = "Graphics Quality Decreased (" .. graphicsSetter.Text .. ")"
				setupGraphicsChangedCallback()
			end
		end)
	end
	...
	if UserSettings().GameSettings:InStudioMode() then studioCheckbox.Text = "X" end
	if hasGraphicsSlider then
