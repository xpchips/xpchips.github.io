...
local function createGameSettingsMenu(baseZIndex, shield)
	...
	if hasGraphicsSlider then
		...
		if game.CoreGui.Version >= 8 then
			UserSettings().GameSettings.GraphicsQualityChanged:connect(function(newQualityLevel)
				if not game.Players["LocalPlayer"] then return end -- don't worry about it unless we are playing a game
				
				if newQualityLevel == Enum.QualityLevel.Automatic then
					setAutoGraphicsGui(true)
				else
					local qualityNum = tonumber(string.sub(tostring(newQualityLevel),-2))
					listenToGraphicsLevelChange = false
						graphicsLevel.Value = math.floor((qualityNum/(settings().Rendering:GetMaxQualityLevel() - 1)) * GraphicsQualityLevels)
						graphicsSetter.Text = tostring(graphicsLevel.Value)
						UserSettings().GameSettings.SavedQualityLevel = graphicsLevel.Value
