...
function initializeDevConsole()
	...
	local minimumSize = Vector2.new(230, 180)
	...
	function refreshTextHolder()
		...
		for i = 1, #messageList do
			...
			message.Position = UDim2.new(0, 5, 0, 14 * (i - 1))
	...
	function repositionList()
		...
		if (currentConsole == LOCAL_CONSOLE) then
			localOffset = math.min(math.max(localOffset, 0), (14 * #localMessageList) - Dev_Container.Body.TextBox.AbsoluteSize.Y)
			Dev_TextHolder.Size = UDim2.new(1, 0, 0, 14 * #localMessageList)
			
		elseif (currentConsole == SERVER_CONSOLE) then
			serverOffset = math.min(math.max(serverOffset, 0), (14 * #serverMessageList) - Dev_Container.Body.TextBox.AbsoluteSize.Y)
			Dev_TextHolder.Size = UDim2.new(1, 0, 0, 14 * #serverMessageList)
		end
		...
		if ratio >= 1 then
			...
			if (currentConsole == LOCAL_CONSOLE) then
				Dev_TextHolder.Position = UDim2.new(0, 0, 1, 0 - (14 * #localMessageList))
			elseif (currentConsole == SERVER_CONSOLE) then
				Dev_TextHolder.Position = UDim2.new(0, 0, 1, 0 - (14 * #serverMessageList))
			end
			...
			if (currentConsole == LOCAL_CONSOLE) then
				Dev_TextHolder.Position = UDim2.new(0, 0, 1, 0 - (14 * #localMessageList) + localOffset)
			elseif (currentConsole == SERVER_CONSOLE) then
				Dev_TextHolder.Position = UDim2.new(0, 0, 1, 0 - (14 * #serverMessageList) + serverOffset)
			end
